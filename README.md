...
* 22022.2.17 增添/修改/删除单个圣遗物功能：ideles大佬已经更新了选择导出功能及圣遗物修改功能，单个圣遗物变动不用再扫描一遍了。我在他的基础上
  * 优化了圣遗物卡片的显示，限制图片大小保证不会变形，将当前词条数移动至最小词条数位置（平常不太会看最小词条数且与当前词条数差别不会很大）
  * 加入了新的评分机制（主词条依赖算法和主词条评分），放在原当前词条数位置，这个评分机制在仅考虑一种属性时与期望一致，但对于所有类型的圣遗物可以同时考虑攻击生命防御，在圣遗物过多时比较圣遗物的好坏更有优势
  * 优化了权重界面的显示，高级权重设置界面更友好，增加了主属性权重等参数配合新评分机制
  * 增加了圣遗物删除功能，当狗粮喂掉的圣遗物可以直接删除而不需要重新扫描一遍
